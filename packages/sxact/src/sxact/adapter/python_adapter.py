"""PythonAdapter — concrete adapter backed by xact.xcore (Julia-via-juliacall).

xact.xcore is a Python API over Julia XCore.jl.  Evaluation therefore goes
...
class PythonAdapter(TestAdapter[_PythonContext]):
    """Concrete adapter for the Python/xact.xcore backend.
    ...
    the xact.xcore Python package as the test subject.
    ...
    def initialize(self) -> _PythonContext:
        try:
            ...
            raise AdapterError(f"xact.xcore unavailable: {exc}") from exc
